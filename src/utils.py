...
"""source: https://github.com/benedekrozemberczki/SimGNN/blob/master/src/utils.py"""
import json
import math
...
def calculate_normalized_ged(data):
    ...
    return norm_ged
def reverse_normalized_ged(norm_ged,data):
    """
    Calculating the normalized GED for a pair of graphs.
    :param data: Data table.
    :return norm_ged: Normalized GED score.
    """
    ged = norm_ged*(0.5*(len(data["labels_1"])+len(data["labels_2"])))
    return float(ged)
